...
const IfcViewer: React.FC<IfcViewerProp> = ({
  viewerRef,
  wasmPath,
  option = {},
  style = {},
  ...
  const ifcOnDoubleClick = async () => {
    if (viewerRef.current) {
      ...
      if (!item) return;
      const props = await viewerRef.current.IFC.getProperties(
        item.modelID,
        item.id,
        true,
        false,
  ...
  const ifcOnMouseMove = async () => {
    if (viewerRef.current) {
      await viewerRef.current.IFC.selector.prePickIfcItem();
    }
  };
